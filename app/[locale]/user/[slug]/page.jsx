...
export async function generateMetadata({ params, searchParams }, parent) {
  ...
  // fetch data
  const response = await fetch(`https://al-kora.com/en/api/user/${params.slug}`);
  ...
  return {
    ...
    image: 'https://al-kora.com' + data.profile.avatar,
    alternates: {
      canonical: '/user/' + user.username,
    },
    openGraph: {
      title: `${user.first_name} ${user.last_name} (@${user.username})`,
      images: ['https://al-kora.com' + data.profile.avatar],
      url: '/user/' + user.username,
      type: 'website',
      image: {
        url: 'https://al-kora.com' + data.profile.avatar,
...
export default async function Profile(props) {
  ...
  const response = await fetch(`https://al-kora.com/en/api/user/${props.params.slug}`);
   if (response.status === 404) {
     return notFound()
   }
  const data = await response.json()
            ...
            <div className='card'>
              <div className='card-body'>
                <div className='d-flex flex-column align-items-center text-center'>
                  <img
                    src={`https://al-kora.com${profile.avatar}`}
