...
export async function generateMetadata({ params, searchParams }, parent) {
    ...
    // fetch data
    const response = await fetch(`https://al-kora.com/en/api/books/category/${slug}`);
    if (!response.ok) {
        return notFound()
    }
...
export default async function BookPage(props) {
  ...
  return (
      <div className='container'>
          <Books url={`https://al-kora.com/${locale}/api/books/category/${slug}`} locale={locale} category={true}/>
      </div>
  )
}
