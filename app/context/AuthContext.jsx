...
export const AuthProvider = ({ children }) => {
  ...
  const getUser = async () => {
    if (DecodeToken !== null) {
      const response = await fetch(
        `https://al-kora.com/en/api/user/${DecodeToken.username}`
      )
  ...
  const login = async (username, password) => {
    ...
    try {
      const response = await fetch('https://al-kora.com/en/api/token/', {
        method: 'POST',
        body: data,
        headers: {
  ...
  const register = async (event) => {
    ...
    await fetch('https://al-kora.com/en/api/register', {
      method: 'POST',
      body: data,
    })
      .then((respons) => {
